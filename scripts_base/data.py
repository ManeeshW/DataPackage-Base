...
class Data:
    ...
    def ypr_to_R(self, ypr):
        ...
        R[0, 1] = np.cos(ypr[0]) * np.sin(ypr[2]) * np.sin(ypr[1]) \
            - np.cos(ypr[2]) * np.sin(ypr[0])
        R[0, 2] = np.sin(ypr[0]) * np.sin(ypr[2]) \
            + np.cos(ypr[0]) * np.cos(ypr[2]) * np.sin(ypr[1])
        R[1, 0] = np.cos(ypr[1]) * np.sin(ypr[0])
        R[1, 1] = np.cos(ypr[0]) * np.cos(ypr[2]) \
            + np.sin(ypr[0]) * np.sin(ypr[2]) * np.sin(ypr[1])
        R[1, 2] = np.cos(ypr[2]) * np.sin(ypr[0]) * np.sin(ypr[1]) \
            - np.cos(ypr[0]) * np.sin(ypr[2])
        R[2, 0] = - np.sin(ypr[1])
